...
export default function SearchPage({ onPostClick }) {
  ...
  useEffect(() => {
    ...
    const match = q ? [null, encodeURIComponent(q)] : null;
    if (match) {
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-4 md:px-8 py-4 sm:py-8 relative z-10">
        ...
        >
          ...
          >
            ...
            <span className="text-xs sm:text-sm font-medium text-primary">Powered by Fuse.js</span>
          </motion.div>

          <motion.h1
            className="text-3xl sm:text-4xl md:text-6xl font-extrabold mb-3 sm:mb-4 bg-gradient-to-r from-primary via-secondary to-primary bg-clip-text text-transparent"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          <motion.p
            className="text-base sm:text-lg text-muted max-w-2xl mx-auto px-2"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
          ...
          >
              ...
              <div className="bg-card rounded-xl border border-border p-4 sm:p-6">
                <div className="flex items-center justify-between mb-3 sm:mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-bold text-sm sm:text-base text-text">Recent Searches</h3>
                  </div>
                  <button
            ...
            <div className="bg-card rounded-xl border border-border p-4 sm:p-6">
              <div className="flex items-center gap-2 mb-3 sm:mb-4">
                <TrendingUp size={18} className="text-secondary sm:w-5 sm:h-5" />
                <h3 className="font-bold text-sm sm:text-base text-text">Popular Topics</h3>
              </div>
              <div className="flex flex-wrap gap-2">
        ...
        {searchResults.length > 0 && (
          ...
          >
            {/* Sort Options */}
            <div className="flex flex-wrap items-center justify-between gap-3 sm:gap-4 bg-card rounded-xl border border-border p-3 sm:p-4">
              <div className="flex items-center gap-2">
                <Target size={16} className="text-primary sm:w-[18px] sm:h-[18px]" />
                <span className="text-xs sm:text-sm font-medium text-text">Sort by:</span>
              </div>
              <div className="flex gap-1.5 sm:gap-2 flex-wrap">
                <button
                  onClick={() => setSortBy('relevance')}
                  className={`px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition-all flex items-center gap-1.5 sm:gap-2 ${sortBy === 'relevance'
                ...
                <button
                  ...
                  className={`px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition-all flex items-center gap-1.5 sm:gap-2 ${sortBy === 'date'
                    ? 'bg-primary text-white'
                    : 'bg-background text-text hover:bg-primary/10'
                    }`}
                >
                ...
                <button
                  ...
                  className={`px-2.5 sm:px-4 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition-all flex items-center gap-1.5 sm:gap-2 ${sortBy === 'title'
                    ? 'bg-primary text-white'
                    : 'bg-background text-text hover:bg-primary/10'
                    }`}
                >
                  <SortAsc size={12} className="sm:w-3.5 sm:h-3.5" />
                  Title
                </button>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 bg-card rounded-xl border border-border p-4">
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all ${selectedCategory === null
                  ? 'bg-primary text-white'
                  : 'bg-background text-text hover:bg-primary/10'
                  }`}
              >
              ...
              {categories.map(category => {
                ...
                return (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all ${selectedCategory === category
                      ? 'bg-primary text-white'
                      : 'bg-background text-text hover:bg-primary/10'
                      }`}
                  >
          ...
          <>
            ...
            {filteredAndSortedResults.length > 0 ? (
              ...
              <motion.div
                ...
                className="bg-card rounded-xl border border-border p-8 sm:p-16 text-center"
              >
                <SearchIcon size={48} className="mx-auto mb-4 text-muted sm:w-16 sm:h-16" />
                <h3 className="text-xl sm:text-2xl font-bold mb-2 text-text">
                  No results found
                </h3>
                <p className="text-sm sm:text-base text-muted mb-4 sm:mb-6 max-w-md mx-auto">
