...
function LessonDetailPage({ lesson, onBack, onSaveTask, onGoToNextLesson }) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-3 sm:px-4 py-4 sm:py-8">
      ...
      <button
        onClick={onBack}
        className="flex items-center gap-2 text-teal-500 hover:text-teal-600 mb-4 sm:mb-6 transition-colors text-sm sm:text-base"
      >
      ...
      {/* Lesson Header */}
      <div className="bg-white dark:bg-stone-800 rounded-lg shadow-md p-4 sm:p-6 mb-4 sm:mb-6">
        <div className="text-xs sm:text-sm text-stone-500 dark:text-stone-400 mb-1">{categoryTitle}</div>
        <h1 className="text-xl sm:text-2xl font-bold text-stone-800 dark:text-stone-100">{title}</h1>
      </div>
      ...
      {/* Practical Task */}
      {task && (
        <div className="bg-gradient-to-br from-teal-50 to-orange-50 dark:from-teal-900/20 dark:to-orange-900/20 rounded-lg shadow-md p-4 sm:p-6 border-2 border-teal-500 dark:border-teal-700">
          <h2 className="text-lg sm:text-xl font-bold text-stone-800 dark:text-stone-100 mb-2 flex items-center gap-2">
            <Target className="text-teal-500 shrink-0" size={20} />
            {task.title}
          </h2>
          <p className="text-sm sm:text-base text-stone-700 dark:text-stone-300 mb-4 leading-relaxed">{task.description}</p>
      ...
      <div className="mt-6 sm:mt-8">
        <button
          ...
          className={`w-full flex items-center justify-center gap-2 px-4 sm:px-6 py-3 sm:py-4 rounded-lg font-bold text-base sm:text-lg shadow-md transition-all transform hover:scale-[1.02] ${
            isComplete
      ...
      {onGoToNextLesson && (
        <div className="mt-3 sm:mt-4 text-center">
          <button
            ...
            className="w-full bg-orange-500 hover:bg-orange-600 text-white font-bold py-3 px-6 sm:px-8 rounded-lg shadow-md transition-all transform hover:scale-105 flex items-center justify-center gap-2 text-sm sm:text-base"
          >
