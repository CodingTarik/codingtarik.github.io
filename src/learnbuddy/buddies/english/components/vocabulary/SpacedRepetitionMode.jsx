...
function SpacedRepetitionMode({ deck, onBack }) {
  ...
  const handleRating = (rating) => {
    ...
    updateCardLocally(deck.id, cardIndex, updatedCard, isLocal);
    updatePendingCount();
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            disabled={syncing}
            className={`p-2 rounded-lg transition-colors disabled:opacity-50 hover:bg-stone-200 dark:hover:bg-stone-700 ${pendingCount > 0 ? 'text-blue-500' : 'text-green-500'
              }`}
            title={pendingCount > 0
              ? (language === 'en' ? 'Upload review progress' : 'Lernfortschritt hochladen')
      ...
      {/* Flashcard */}
      <div
        className={`bg-gradient-to-br from-white to-stone-50 dark:from-stone-800 dark:to-stone-900 rounded-2xl p-12 shadow-2xl border-2 border-stone-200 dark:border-stone-700 min-h-[400px] flex flex-col items-center justify-center cursor-pointer transition-all duration-300 ${isFlipping ? 'scale-95 opacity-50' : 'scale-100 opacity-100'
          }`}
        onClick={() => {
          setIsFlipping(true);
          sounds.playCardFlip();
