...
function CardManager({ deck, onBack }) {
  ...
  // Determine if this is a local deck (no spreadsheet URL or explicitly set to local)
  const isLocal = deck.storageMode === 'local' || (!deck.storageMode && !deck.scriptUrl);
  ...
  const handleAdd = () => {
    ...
    // Add locally
    const success = addCardLocally(deck.id, newCard, isLocal);
    ...
    if (success) {
      ...
      const message = language === 'en'
        ? 'Card added locally.' + (isLocal ? '' : ' Remember to sync!')
        : 'Karte lokal hinzugefügt.' + (isLocal ? '' : ' Vergiss nicht zu synchronisieren!');
  ...
  const handleEdit = () => {
    ...
    const success = updateCardLocally(deck.id, cardIndex, updatedCard, isLocal);
  ...
  const handleDelete = (card) => {
    ...
    const success = deleteCardLocally(deck.id, cardIndex, isLocal);
    ...
    if (success) {
      ...
      toast.success(language === 'en'
        ? 'Card deleted!' + (isLocal ? '' : ' Remember to sync.')
        : 'Karte gelöscht!' + (isLocal ? '' : ' Vergiss nicht zu synchronisieren.'),
        { icon: '🗑️' }
      );
    } else {
  ...
  return (
    <div className="max-w-6xl mx-auto pb-8">
        ...
        <div className="flex items-center gap-2">
          ...
          {!isLocal && (
            <button
              ...
              disabled={syncing}
              className={`flex items-center gap-2 px-4 py-2 font-bold rounded-xl hover:shadow-lg transition-all disabled:opacity-50 ${pendingCount > 0
      ...
      {pendingCount > 0 && !syncing && !isLocal && (
            ...
            <div className="absolute right-0 mt-2 bg-white dark:bg-stone-800 rounded-xl shadow-2xl border-2 border-stone-200 dark:border-stone-700 p-2 z-20 min-w-[200px] sort-menu-container">
              {[
                ...
                <button
                  ...
                  className={`w-full text-left px-3 py-2 rounded-lg hover:bg-stone-100 dark:hover:bg-stone-700 transition-colors flex items-center justify-between ${sortBy === option.key ? 'bg-purple-100 dark:bg-purple-900/20 text-purple-700 dark:text-purple-300' : 'text-stone-700 dark:text-stone-300'
                    }`}
                >
