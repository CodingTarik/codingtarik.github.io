...
const WORD_LISTS = {
  random: {
    label: { en: 'Random', de: 'Zufall' },
    words: {
      en: ['House', 'Tree', 'Computer', 'Pizza', 'Sun', 'Cat', 'Car', 'Banana', 'Robot', 'Ghost', 'School', 'Fire', 'Water', 'Moon', 'Fish', 'Bird', 'Book', 'Pen', 'Lamp', 'Phone', 'Glasses', 'Clock', 'Shoe', 'Hat', 'Ball', 'Table', 'Chair', 'Bed'],
      de: ['Haus', 'Baum', 'Computer', 'Pizza', 'Sonne', 'Katze', 'Auto', 'Banane', 'Roboter', 'Geist', 'Schule', 'Feuer', 'Wasser', 'Mond', 'Fisch', 'Vogel', 'Buch', 'Stift', 'Lampe', 'Telefon', 'Brille', 'Uhr', 'Schuh', 'Hut', 'Ball', 'Tisch', 'Stuhl', 'Bett'],
    }
  },
  animals: {
    label: { en: 'Animals', de: 'Tiere' },
    words: {
      en: ['Elephant', 'Giraffe', 'Lion', 'Zebra', 'Penguin', 'Eagle', 'Shark', 'Whale', 'Dolphin', 'Kangaroo', 'Koala', 'Panda', 'Bat', 'Spider', 'Snake', 'Turtle', 'Crocodile', 'Rhino', 'Hippo', 'Gorilla'],
      de: ['Elefant', 'Giraffe', 'Löwe', 'Zebra', 'Pinguin', 'Adler', 'Hai', 'Wal', 'Delfin', 'Känguru', 'Koala', 'Panda', 'Fledermaus', 'Spinne', 'Schlange', 'Schildkröte', 'Krokodil', 'Nashorn', 'Nilpferd', 'Gorilla'],
    }
  },
  food: {
    label: { en: 'Food', de: 'Essen' },
    words: {
      en: ['Hamburger', 'Fries', 'Spaghetti', 'Sushi', 'Taco', 'Ice Cream', 'Cake', 'Donut', 'Cookie', 'Chocolate', 'Apple', 'Strawberry', 'Pineapple', 'Watermelon', 'Cherry', 'Peach', 'Grape', 'Lemon', 'Avocado', 'Tomato'],
      de: ['Hamburger', 'Pommes', 'Spaghetti', 'Sushi', 'Taco', 'Eis', 'Kuchen', 'Donut', 'Keks', 'Schokolade', 'Apfel', 'Erdbeere', 'Ananas', 'Melone', 'Kirsche', 'Pfirsich', 'Traube', 'Zitrone', 'Avocado', 'Tomate'],
    }
  },
  tech: {
    label: { en: 'Tech', de: 'Technik' },
    words: {
      en: ['Laptop', 'Smartphone', 'Tablet', 'Headphones', 'Keyboard', 'Mouse', 'Monitor', 'Printer', 'Camera', 'Microphone', 'Television', 'Console', 'Controller', 'Cable', 'Battery', 'Robot', 'Drone', 'Satellite', 'Rocket'],
      de: ['Laptop', 'Smartphone', 'Tablet', 'Kopfhörer', 'Tastatur', 'Maus', 'Monitor', 'Drucker', 'Kamera', 'Mikrofon', 'Fernseher', 'Konsole', 'Controller', 'Kabel', 'Batterie', 'Roboter', 'Drohne', 'Satellit', 'Rakete'],
    }
  },
};
...
const T = {
  en: {
    ...
    guessInChat: 'Guess the word in the chat!',
    cantTell: "You can't reveal the word!",
    ...
    hostChoosing: 'Host is choosing settings...',
    joined: (name) => `${name} joined!`,
    yourTurn: (word) => `Your turn! Draw: ${word}`,
    isDrawingNow: (name) => `${name} is drawing now!`,
    guessedWord: (name) => `🎉 ${name} guessed the word!`,
    timeUp: (word) => `Time's up! Word: ${word}`,
  ...
  de: {
    chooseStyle: 'Wähle deinen Style',
    yourName: 'Dein Name',
    ...
    draw: 'Zeichne',
    isDrawing: 'zeichnet',
    guessInChat: 'Rate das Wort im Chat!',
    cantTell: 'Du darfst nicht verraten!',
    yourGuess: 'Dein Tipp...',
    gameOver: 'Spiel Vorbei!',
    winnerIs: 'Der Gewinner ist',
    backToMenu: 'Zurück zum Menü',
    category: 'Kategorie',
    write: 'Schreiben...',
    waitingForPlayers: 'Warte auf Spieler...',
    startGame: 'Spiel starten',
    hostChoosing: 'Host wählt Einstellungen...',
    joined: (name) => `${name} ist beigetreten!`,
    yourTurn: (word) => `Du bist dran! Zeichne: ${word}`,
    isDrawingNow: (name) => `${name} zeichnet jetzt!`,
    guessedWord: (name) => `🎉 ${name} hat das Wort erraten!`,
    timeUp: (word) => `Zeit um! Wort: ${word}`,
    enterName: 'Name eingeben!',
    enterCode: 'Code eingeben!',
    roomNotFound: 'Raum nicht gefunden! Code prüfen.',
...
export default function DoodleDash() {
    ...
    return new Promise((resolve, reject) => {
      ...
      peer.on('error', (err) => {
        ...
        if (err.type === 'peer-unavailable') {
          alert(T[langRef.current].roomNotFound);
    ...
    switch (data.type) {
      case 'join_request': // HOST only
        ...
        if (!state.players.find(p => p.name === data.name)) {
          ...
          playSound('join');
          addSystemMessage(T[langRef.current].joined(newPlayer.name));
        }
        break;
      ...
      case 'start_round':
        ...
        if (isMeDrawer) {
          ...
          addSystemMessage(T[langRef.current].yourTurn(data.word));
        } else {
          setMaskedWord('_ '.repeat(data.wordLength));
          addSystemMessage(T[langRef.current].isDrawingNow(data.drawer));
        }
        playSound('turn');
        break;
      ...
      case 'correct_guess':
        addSystemMessage(T[langRef.current].guessedWord(data.winner));
      ...
      case 'reveal_word':
        addSystemMessage(T[langRef.current].timeUp(data.word));
    ...
    if (state.isHost) {
      ...
      if (msg.author !== state.currentDrawer) {
        ...
        if (guess === target && !state.guessedPlayers.includes(msg.author)) {
          ...
          addSystemMessage(T[langRef.current].guessedWord(msg.author));
          playSound('correct');
  ...
  const startNextRound = () => {
    ...
    const wordList = (WORD_LISTS[selectedCategory] || WORD_LISTS['random']).words[langRef.current];
    ...
    if (nextDrawer === state.myName) {
      setCurrentWord(word);
      addSystemMessage(T[langRef.current].yourTurn(word));
    } else {
      setCurrentWord('');
      setMaskedWord('_ '.repeat(word.length));
      addSystemMessage(T[langRef.current].isDrawingNow(nextDrawer));
    }
    ...
    timerRef.current = setInterval(() => {
      ...
      if (t === 0) {
        ...
        addSystemMessage(T[langRef.current].timeUp(word));
        setMaskedWord(word);
  ...
  useEffect(() => {
    const m = window.location.pathname.match(/\/games\/doodledash\/([A-Z0-9]+)/i);
    if (m) setRoomCode(m[1].toUpperCase());
  }, []);

  const [linkCopied, setLinkCopied] = useState(false);
  const shareInviteLink = () => {
    const url = `${window.location.origin}/games/doodledash/${roomCode}`;
    if (navigator.share) {
      navigator.share({ title: 'DoodleDash', text: 'Join my DoodleDash game!', url }).catch(() => {});
    } else {
  ...
  const createRoom = async () => {
    if (!myName.trim()) return alert(T[langRef.current].enterName);
    try {
      await initPeer();
      setIsHost(true);
  ...
  const joinRoom = async () => {
    if (!myName.trim()) return alert(T[langRef.current].enterName);
    if (!roomCode.trim()) return alert(T[langRef.current].enterCode);
    try {
      ...
      conn.on('error', () => {
        alert(T[langRef.current].connectionFailed);
        setConnectionStatus('disconnected');
      });
    } catch (e) { console.error(e); }
  };
      ...
      <header className="bg-white border-b border-indigo-100 p-3 shadow-sm shrink-0 z-10">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={() => setLang(l => { const next = l === 'en' ? 'de' : 'en'; langRef.current = next; return next; })}
              className="px-2.5 py-1 rounded-full bg-slate-100 text-xs font-bold text-slate-500 hover:bg-indigo-100 hover:text-indigo-600 transition"
            >
              {lang === 'en' ? '🇬🇧 EN' : '🇩🇪 DE'}
            </button>
            ...
            >
              <div className="bg-white p-8 rounded-3xl shadow-2xl max-w-md w-full border border-indigo-50">
                <div className="text-center mb-6">
                  <h2 className="text-4xl font-black text-slate-800 mb-2 tracking-tight">{t.startGame}</h2>
                  <p className="text-slate-400 font-medium">{t.chooseStyle}</p>
                </div>
                ...
                <div className="space-y-5">
                  ...
                  <div className="pt-4 space-y-3">
                    ...
                    >
                      <Play size={20} fill="currentColor" /> {t.createRoom}
                    </button>
              ...
              <div className="bg-white rounded-3xl shadow-2xl max-w-4xl w-full h-[600px] overflow-hidden border border-indigo-50 flex">
                ...
                <div className="w-2/3 p-8 flex flex-col">
                  <div className="flex justify-between items-start mb-8">
                    <div>
                      ...
                      <button onClick={shareInviteLink} className="mt-2 inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 font-semibold rounded-lg text-xs transition">
                    ...
                    {isHost && (
                      <div className="flex flex-col items-end gap-1">
                        ...
                        >
                          {Object.keys(WORD_LISTS).map(c => <option key={c} value={c}>{WORD_LISTS[c].label[lang]}</option>)}
                        </select>
                      </div>
                    )}
                  </div>
                      ...
                      {players.map((p, i) => (
                        ...
                        >
                          ...
                          <div className="flex flex-col">
                            ...
                            {p.name === myName && <span className="text-[10px] text-indigo-500 font-bold uppercase">{t.you}</span>}
                          </div>
                        </motion.div>
                      ))}
                    </div>
                  ...
                  <div className="mt-6 pt-6 border-t border-slate-100">
                    {isHost ? (
                      ...
                      >
                        ...
                        {players.length < 2 ? t.waitingForPlayers : t.startGame}
                      </button>
                    ) : (
                      <div className="text-center p-4 bg-slate-50 rounded-2xl border border-slate-100 text-slate-500 font-medium flex items-center justify-center gap-2">
                        <Loader2 size={20} className="animate-spin text-indigo-500" /> {t.hostChoosing}
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="w-1/3 bg-slate-50 border-l border-slate-100 flex flex-col">
                  ...
                  <form onSubmit={sendChat} className="p-3 border-t border-slate-200">
                    <input name="msg" type="text" placeholder={t.write} className="w-full p-2 rounded-xl border border-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none text-sm" autoComplete="off"/>
                  </form>
                </div>
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="w-full md:w-72 flex flex-col gap-4 shrink-0">
                ...
                <div className="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 flex-1 overflow-y-auto">
                  <h3 className="font-bold text-xs text-slate-400 uppercase tracking-wider mb-4 ml-1">{t.players}</h3>
                      ...
                      <div key={p.name} className={`flex items-center justify-between p-2.5 rounded-2xl ${p.name === currentDrawer ? 'bg-indigo-50 ring-2 ring-indigo-100' : 'bg-slate-50'} transition-all`}>
                        <div className="flex items-center gap-3 overflow-hidden">
                           ...
                           <div className="flex flex-col">
                             ...
                             <div className="text-[10px] font-bold text-slate-400">{p.name === currentDrawer ? t.drawing : p.name === myName ? t.you : ''}</div>
                           </div>
                        </div>
                ...
                <div className="bg-white rounded-t-3xl p-4 border-b border-slate-100 flex justify-between items-center shadow-sm relative z-10">
                   ...
                   {currentDrawer === myName ? (
                     <div className="text-center absolute left-1/2 -translate-x-1/2">
                        <span className="text-[10px] font-bold text-indigo-500 uppercase tracking-widest block mb-1">{t.draw}</span>
                        <span className="text-2xl md:text-3xl font-black text-slate-800 tracking-wide">{currentWord}</span>
                     </div>
                   ) : (
                     <div className="text-center absolute left-1/2 -translate-x-1/2">
                        <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block mb-1">{currentDrawer} {t.isDrawing}</span>
          ...
          {gameState === 'gameover' && (
             ...
             >
               <div className="bg-white rounded-[2rem] shadow-2xl p-12 text-center max-w-md w-full border border-white/50">
                 ...
                 <p className="text-slate-500 mb-8 font-medium">{t.winnerIs}</p>
